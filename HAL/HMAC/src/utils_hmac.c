...
void utils_hmac_sha256(const char *msg, int msg_len, char *digest, const char *key, int key_len)
{
    ...
    mbedtls_sha256_update(&context, k_ipad, KEY_IOPAD_SIZE);            /* start with inner pad */
    mbedtls_sha256_update(&context, (unsigned char *) msg, msg_len);    /* then text of datagram */
    mbedtls_sha256_finish(&context, out);                               /* finish up 1st pass */
    mbedtls_sha256_free( &context );

    /* perform outer SHA */
    mbedtls_sha256_init(&context);                              /* init context for 2nd pass */
    mbedtls_sha256_starts(&context,0);                            /* setup context for 2nd pass */
    mbedtls_sha256_update(&context, k_opad, KEY_IOPAD_SIZE);    /* start with outer pad */
    mbedtls_sha256_update(&context, out, SHA256_DIGEST_SIZE);     /* then results of 1st hash */
    mbedtls_sha256_finish(&context, out);                       /* finish up 2nd pass */
    mbedtls_sha256_free( &context );
